<template>
    <NuxtLayout name="default">
        <template v-slot:headerTitle>Meus Fluxos</template>
        <template v-slot:headerDescription>Controle financeiro mensal de entradas e saídas</template>
        ...
        <div class="w-full flex flex-col gap-4">
            <!-- Cards Grid -->
            <div v-if="(cashFlows || [])?.length" class="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
                <NuxtLink
            ...
            <!-- Empty State -->
            <div v-else class="bg-bg-card border border-border rounded-xl p-12 text-center">
            ...
            >
                <div class="bg-bg-card border border-border rounded-xl p-6 w-full max-w-md">
                    ...
                    <form @submit.prevent="createCashFlow" class="space-y-5">
                        <div>
                            <label class="block text-sm font-medium text-text-secondary mb-2">Mês</label>
                            <select
                        ...
                        <div>
                            <label class="block text-sm font-medium text-text-secondary mb-2">Ano</label>
                            <input
                        ...
                        <div>
                            <label class="block text-sm font-medium text-text-secondary mb-2">Saldo Inicial (R$)</label>
                            <input
...
const { data: cashFlows, refresh } = await useFetch<CashFlow[]>('/api/cash-flows');
...
async function createCashFlow() {
    ...
    try {
        ...
        errorMessage.value = error.data?.message || 'Erro ao criar fluxo de caixa';
    } finally {
